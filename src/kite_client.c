...
int xrg_vector_fill_sockbuf(xrg_vector_t **c, sockbuf_t *sockbuf) {
	...
	*c = 0;
	...
	if (xrg_vector_is_compressed(v)) {
		...
		if (ret != nbyte) {
			fprintf(stderr, "xrg_vector_fill_sockbuf: LZ4 decompress_safe byte not match (%d != %d)\n", ret, nbyte);
			return 0;
		}
...
bool kite_result_fill(sockstream_t *ss, kite_result_t *res) {
	...
	do {
		...
		if (sockbuf.msgsz > 0) {
			...
			if (nrow != ret) {
				fprintf(stderr, "VECTOR: number of rows not match between columns (nrow=%d, ret=%d)", nrow, ret);
				goto done;
			}
